...
void main() {
  ...
  runApp(const WalkingCounter());
}

class WalkingCounter extends StatelessWidget {
  const WalkingCounter({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const WalkingCounterHome(),
    );
  }
}

class WalkingCounterHome extends StatefulWidget {
  const WalkingCounterHome({super.key});

  @override
  WalkingCounterHomeState createState() => WalkingCounterHomeState();
}
...
class WalkingCounterHomeState extends State<WalkingCounterHome>
  ...
  Future<void> _initializeData() async {
    try {
      ...
      await provider.checkGoalCompletion('walking');
      setState(() {
        _walkingSteps = int.parse(provider.userData!['walkingSteps']);
  ...
  Future<void> fetchGoals() async {
    try {
      final querySnapshot =
          await FirebaseFirestore.instance.collection('walkingGoals').get();
      ...
      setState(() {
        walkingGoals = []; // Set an empty list to avoid infinite spinner
      });
    }
  }
  ...
  void _toggleCounting() {
    final provider = Provider.of<UserDataProvider>(context, listen: false);
    if (provider.userData!['isWalkingGoalActive'] == 'false') {
      ...
      if (isValidMovement) {
        ...
        if (estimatedSteps > _lastValidStepCount &&
          ...
          if (_walkingSteps % 10 == 0) {
            ...
            provider.checkGoalCompletion('walking').then((_) {
              if (provider.userData!['isWalkingGoalActive'] == 'false') {
                _stopCounting();
                setState(() {
                  _isCounting = false;
  ...
  Future<void> _updateWalkingSteps(int newStepCount) async {
    final provider = Provider.of<UserDataProvider>(context, listen: false);
    await provider.updateSteps('walking', newStepCount);
  }
  ...
  void _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != _selectedDate) {
      ...
      provider.updateUserData({
        'walkingGoalEndDate': DateFormat('yyyy-MM-dd').format(picked),
      });
    }
  }
    ...
    return MaterialApp(
      ...
      home: Consumer<UserDataProvider>(
        builder: (context, userDataProvider, child) {
          bool isWalkingGoalActive =
              userDataProvider.userData!['isWalkingGoalActive'] == 'true';
          String walkingGoal = userDataProvider.userData!['walkingGoal'];
          String walkingGoalEndDateStr =
              userDataProvider.userData!['walkingGoalEndDate'];
          DateTime walkingGoalEndDate = DateTime.parse(walkingGoalEndDateStr);
          bool isEndDateValid = walkingGoalEndDate.isAfter(_currentDate);
          bool isGoalSelectionValid = walkingGoal != '0';
          String currentSteps = userDataProvider.userData!['walkingSteps'];

          // Calculate progress
          double progress = isWalkingGoalActive
              ? (int.parse(currentSteps) / int.parse(walkingGoal))
                  .clamp(0.0, 1.0)
              : 0.0;
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 14),
                            Text(
                              "${NumberFormat('#,###').format(int.parse(walkingGoal))} STEP${int.parse(walkingGoal) == 0 || int.parse(walkingGoal) == 1 ? '' : 'S'}",
                              style: const TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                              ),
                                  ...
                                  : SizedBox(
                                      ...
                                      child: DropdownButtonFormField<String>(
                                        value: walkingGoal == '0'
                                            ? null
                                            : walkingGoal,
                                        onChanged: isWalkingGoalActive
                                            ? null
                                            : (String? newValue) {
                                                if (newValue != null) {
                                                  userDataProvider
                                                      .updateUserData({
                                                    'walkingGoal': newValue,
                                                  });
                                                }
                                              },
                                        items: walkingGoals.map((goal) {
                            ...
                            Center(
                              child: SizedBox(
                                ...
                                child: ElevatedButton(
                                  ...
                                  child: Text(
                                    walkingGoalEndDate != _currentDate &&
                                            walkingGoalEndDate
                                                .isAfter(_currentDate)
                                        ? 'END DATE: ${DateFormat('MMMM d, yyyy').format(walkingGoalEndDate).toUpperCase()}'
                                        : 'PICK DATE: ${DateFormat('MMMM d, yyyy').format(_currentDate).toUpperCase()}',
                                    style: TextStyle(
                                      color: isWalkingGoalActive
                                          ? Colors.white
                                          : Colors.black,
                                      fontSize: 14,
                                    ),
